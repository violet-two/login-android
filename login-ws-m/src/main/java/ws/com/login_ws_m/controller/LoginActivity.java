package ws.com.login_ws_m.controller;
...
import ws.com.login_ws_m.R;
import ws.com.login_ws_m.model.IBaseRetCallback;
import ws.com.login_ws_m.model.impl.LoginModelImpl;
import ws.com.login_ws_m.util.LoginUtil;
import ws.com.login_ws_m.util.MD5Util;
import ws.com.login_ws_m.util.ScreenUtil;
import ws.com.login_ws_m.util.StatusBarUtil;
import ws.com.login_ws_m.util.ToastUtil;
            ...
            public void onFocusChange(View view, boolean b) {
                if (b) {
                    ...
                    if(!inputPhone.matches(isTruePhoneNum)){
                        ToastUtil.show(LoginActivity.this,"手机号格式不正确");
                        user.requestFocus();
                    }
                } else {
                    System.out.println("失去焦点");
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.btn_login:
                ...
                loginModel = new LoginModelImpl();
                loginModel.login(params, new IBaseRetCallback<LoginUtil>() {
                    @Override
                    public void onSucceed(Response<LoginUtil> response) {
                        LoginUtil body = response.body();
