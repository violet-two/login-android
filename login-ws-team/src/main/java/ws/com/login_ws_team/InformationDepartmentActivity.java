...
import ws.com.login_ws_team.presenter.InformationPresenter;
...
public class InformationDepartmentActivity extends BaseActivity<InformationPresenter, IInformationView> implements IInformationView {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        informationListRV.post(() -> {
            itemNum = getInformationListRVHeight();
            hashMap = new HashMap<>();
            hashMap.put("managerPhone", selfPhone);
            hashMap.put("phone", "");
            hashMap.put("startPage",1);
            hashMap.put("pageSize",itemNum);
            initData(hashMap);
        });
        //设置主题
        setTheme();
...
//            InformationDP.informationDP(this, allView, hashMap);
    ...
    private void handlerDownPullUpdate() {
        sr.setEnabled(true);
        //添加事件监听器
        sr.setOnRefreshListener(() -> new Handler().postDelayed(() -> {
            //上拉刷新重置数据
            initData(hashMap);
    ...
    @Override
    public void queryInformation(Response<InformationDPBean> response) {
        InformationDPBean body = response.body();
    ...
    private synchronized void initData(HashMap<String, Object> hashMap) {
        informationPresenter.information(hashMap);
    }
    ...
    private void getLoginData() {
        ...
        if (bundle != null) {
            ...
            Type type = new TypeToken<ArrayList<LoginBean.DataBean>>(){}.getType();
